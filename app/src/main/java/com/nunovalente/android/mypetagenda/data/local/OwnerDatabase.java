package com.nunovalente.android.mypetagenda.data.local;

import android.content.Context;
import android.util.Log;

import androidx.room.Database;
import androidx.room.Room;
import androidx.room.RoomDatabase;

import com.nunovalente.android.mypetagenda.model.Owner;

@Database(entities = {Owner.class}, version = 1, exportSchema = false)
public abstract class OwnerDatabase extends RoomDatabase {

    private static final String TAG = OwnerDatabase.class.getSimpleName();
    private static final Object LOCK = new Object();
    private static final String DATABASE_NAME = "owner_database";
    private static OwnerDatabase sInstance;
    public abstract OwnerDao ownerDao();


    /**
     * Singleton that creates the Database Instance or returns if already created.
     *
     */
    public static OwnerDatabase getInstance(Context context) {
        if (sInstance == null) {
            synchronized (LOCK) {
                Log.d(TAG, "Creating Database instance");
                sInstance = Room.databaseBuilder(context.getApplicationContext(), OwnerDatabase.class, DATABASE_NAME).build();
            }
        }
        Log.d(TAG, "Getting database Instance");
        return sInstance;
    }
}
