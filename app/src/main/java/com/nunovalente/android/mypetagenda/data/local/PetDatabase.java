package com.nunovalente.android.mypetagenda.data.local;

import android.content.Context;
import android.util.Log;

import androidx.room.Database;
import androidx.room.Room;
import androidx.room.RoomDatabase;

import com.nunovalente.android.mypetagenda.model.Pet;

@Database(entities = {Pet.class}, version = 1, exportSchema = false)
public abstract class PetDatabase extends RoomDatabase {

    private static final String TAG = PetDatabase.class.getSimpleName();
    private static final Object LOCK = new Object();
    private static final String DATABASE_NAME = "pet_table";
    private static PetDatabase sInstance;
    public abstract PetDao petDao();

    /**
     * Singleton that creates the Database Instance or returns if already created.
     */
    public static PetDatabase getInstance(Context context) {
        if (sInstance == null) {
            synchronized (LOCK) {
                Log.d(TAG, "Creating Database instance");
                sInstance = Room.databaseBuilder(context.getApplicationContext(), PetDatabase.class, DATABASE_NAME).build();
            }
        }
        Log.d(TAG, "Getting database Instance");
        return sInstance;
    }
}
